...
;;      => (begin
;;           (define temp (call-with-values (lambda () (values 1 2)) vector))
;;           (define-values-helper temp 0 (x y)))
;;      => (begin
;;           (define temp '#(1 2))
;;           (define x (vector-ref temp 0))
;;           (define y (vector-ref temp 1)))
;;   注意: 内部使用 vector 暂存多值，然后逐一定义
(define-syntax define-values
  (syntax-rules ()
    ((define-values (var ...) expr)
     (begin
       (define temp (call-with-values (lambda () expr) vector))
       (define-values-helper temp 0 (var ...))))))

;; ── define-values-helper ──
;; define-values 的递归辅助宏（不直接使用）。
;;   从 vector temp 中按索引逐位提取值并 define 到对应变量。
