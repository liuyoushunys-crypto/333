...
(define (qq-tail-unquote? tail)
  (and (pair? tail) (eq? (car tail) 'unquote)))

(define (qq-tail-unsplice? tail)
  (and (pair? tail) (eq? (car tail) 'unquote-splicing)))
    ...
    ((qq-unsplice? el)
     (let ((v (eval (cadr el) env)))
       (cond
         ...
         ((null? v) items)
         (else (cons v items)))))
    ((and (pair? el) (eq? (car el) 'quasiquote))
     (cons el items))
    (else
      ...
      (if (null? (cdr args))
          ...
          ((lambda (test rest)
             (list 'let (list (list 'temp test))
                   (list 'if 'temp 'temp (cons 'or rest))))
           (car args) (cdr args)))))
...
(define-macro (let . args)
  (if (and (pair? args) (symbol? (car args)))
      ((lambda (name rest)
     ...
     ((lambda (loop)
      ...
      (lambda (loop bs bd)
        (if (null? bs)
            (cons 'begin bd)
            ((lambda (b)
               (list 'let (list b) (loop loop (cdr bs) bd)))
             (car bs))))))
   (car args) (cdr args)))
...
(define-macro (letrec . args)
  ((lambda (bindings body)
     ((lambda (vars vals)
        (cons 'begin
              (append
                (list (list 'let (map (lambda (v) (list v '#f)) vars) '()))
                (map (lambda (v val) (list 'set! v val)) vars vals)
                (list (cons 'let (cons '() body))))))
   ...
   (lambda (loop cls)
     (if (null? cls)
         ...
         ((lambda (cl)
            (if (eq? (car cl) 'else)
                (cons 'begin (cdr cl))
                (if (and (pair? cl) (pair? (cdr cl)) (eq? (cadr cl) '=>))
...
(define (sx-pattern-vars pat)
  (if (symbol? pat)
      (if (eq? pat '_)
          ...
          (if (eq? pat '...) '() (list pat)))
      (if (pair? pat)
          (sx-merge-vars (sx-pattern-vars (car pat))
                         (sx-pattern-vars (cdr pat)))
          '())))
...
(define (sx-accum-ellipsis vars groups base)
  (if (null? vars)
      base
      (let* ((v (car vars))
             (vals (map (lambda (g) (sx-lookup v g)) groups))
             (lst (sx-rev-append vals '())))
        (sx-merge-bindings (cons (cons v lst) '())
  ...
  (cond
    ...
    ((symbol? pat)
     (cond
       ((eq? pat '_) '())
       ((memq pat lits)
        (if (and (symbol? inp) (eq? pat inp)) '() #f))
       (else (list (cons pat inp)))))
    ((not (pair? pat))
     (if (equal? pat inp) '() #f))
    ;; (prefix ...) 或 (prefix ... rest)
...
(define (sx-match-ellipsis-finish prefix rest-pat res lits)
  (let ((in (car res)) (groups (cdr res)))
    (let ((evars (sx-pattern-vars prefix)))
      (if (null? rest-pat)
          (if (null? in)
...
;; 条件: prefix 匹配 in 首元素, 且 (restPat 空 或 restPat 不匹配剩余输入)
;; 返回 (剩余输入 . groups)
(define (sx-match-ellipsis-loop prefix rest-pat in lits groups)
  (if (pair? in)
      (let ((b (sx-match prefix (car in) lits)))
        (if (and b (or (null? rest-pat) (not (sx-match rest-pat in lits))))
...
(define (sx-ellipsis-vars-helper vs bindings acc)
  (if (null? vs)
      ...
      (let ((p (assq (car vs) bindings)))
        (if (and p (or (pair? (cdr p)) (null? (cdr p))))
