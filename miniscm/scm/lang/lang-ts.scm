...
(define-syntax let (syntax-rules (=) ((_ name = val) (define name val)) ((_ name) (define name #f))))
...
(function filter-demo (arr)
  (let evens = (filter arr (lambda (x) (zero? #{x % 2}))))
  (console.log "evens:" evens))
(filter-demo (list 1 2 3 4 5 6))

(function sum-demo (arr)
  (let total = (reduce (lambda (x acc) #{x + acc}) 0 arr))
...
;;   (let nums = (iota n 1))
;;   (let result = (reduce (lambda (x acc) #{x * acc}) 1 nums))
...
;;   (let evens = (filter arr (lambda (x) (zero? #{x % 2}))))
;;   (let squares = (map (lambda (x) #{x * x}) evens))
;;   (let total = (reduce (lambda (x acc) #{x + acc}) 0 squares))
